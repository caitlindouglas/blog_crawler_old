...
module MechanizeAdapter
  ...
  def safe_request url, options, &block
    begin
      try_n_times do
        yield page_preprocess fetcher { @agent.get url, options[:parameters] || [], options[:referer], options[:headers] || {} }
      end
    rescue => exception
  ...
  def request url, options = {}
    begin
      Timeout::timeout(120){ page_preprocess @agent.get url, options[:parameters] || [], options[:referer], options[:headers] || {} }
